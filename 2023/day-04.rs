...
PART 1
  ...
  better naming scheme. Tell you what: if you can help me with something quick, I'll let you borrow
  my boat and you can go visit the gardener. I got all these scratchcards as a gift, but I can't
  ...
  you have (83, 86, 6, 31, 17, 9, 48, and 53). Of the numbers you have, four of them (48, 83, 17, and
  86) are winning numbers! That means card 1 is worth 8 points (1 for the first match, then doubled
...
const USAGE: &str = "Usage: ./day-04 <part(1|2)> [test]";

fn parse_line(line: &str) -> (HashSet<u32>, Vec<u32>) {
    let line = line.split(": ").last().unwrap();
    let mut two_parts = line.split(" | ");
    let winning_numbers: HashSet<u32> = two_parts
        .next()
        .unwrap()
        .split_whitespace()
        .map(|s| s.parse::<u32>().unwrap())
        .collect();
    let numbers: Vec<u32> = two_parts
        .next()
        .unwrap()
        .split_whitespace()
        .map(|s| s.parse::<u32>().unwrap())
...
fn solve_part_2(input: &str) -> u32 {
    let max_card_id = input.lines().count();
    let mut card_counts: Vec<u32> = vec![1; max_card_id];
    for (id, line) in input.lines().enumerate() {
        ...
        let matches = numbers
            ...
            .filter(|n| winning_numbers.contains(n))
            .count();
        let count = card_counts[id];
        for new_cards in id + 1..min(id + matches + 1, max_card_id) {
            card_counts[new_cards] += count;
        }
    }
